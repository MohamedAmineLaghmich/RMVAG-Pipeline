# ==============================================================================
# INFERENCE SCRIPT: SERRES MAP GENERATION
# Architecture: 3 Months x 6 Features = 18 Bands (With NDWI Mask)
# ==============================================================================
...
CROP_TYPE, N_MONTHS, N_FEATURES, TOTAL_BANDS = "Serres", 3, 6, 18
...
input_tiles = glob.glob(f'/content/drive/MyDrive/**/{DRIVE_FOLDER}/Serres_Stack*.tif', recursive=True)
        ...
        with rasterio.open(out_path, 'w', **meta) as dst:
            for _, window in src.block_windows(1):
                ...
                if valid.shape[0] > 0:
                    ...
                    preds = (model.predict(reshaped, batch_size=8192, verbose=0)[:, 1] >= 0.98).astype(np.uint8)
                    
                    # WATER MASK FIX (NDWI is Feature index 3)
                    raw_reshaped = valid.reshape((-1, N_MONTHS, N_FEATURES))
                    preds[np.mean(raw_reshaped[:, :, 3], axis=1) > 0] = 0 
